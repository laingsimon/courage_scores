...
export function valueChanged<T>(
    ...
    set:
        ...
        | ((value: T) => UntypedPromise)
        | ((value: T | null) => UntypedPromise),
    nullIf?: string,
) {
 ...
 * */
/* eslint-disable @typescript-eslint/no-explicit-any */
export function propChanged<T>(
    get: T,
    set: Dispatch<SetStateAction<T>> | ((value: T) => UntypedPromise),
    prop?: string,
): (x: any, y?: any) => UntypedPromise {
    /* eslint-disable @typescript-eslint/no-explicit-any */
 ...
 * */
export function stateChanged<T>(
    set:
        | Dispatch<SetStateAction<T>>
        | ((value: T) => UntypedPromise)
        | ((value: T) => void),
...
export function handleChange(
    handler?: (name: string, value: any) => UntypedPromise,
) {
    if (!handler) {
        // prevent updates
        return () => false;
