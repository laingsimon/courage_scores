...
}: IWidescreenMatchStatisticsProps) {
    const [oneDartAverage, setOneDartAverage] = useState<boolean>(
        initialOneDartAverage || false,
    );
    const finished: boolean =
        homeScore >= numberOfLegs / 2.0 ||
        (awayScore || 0) >= numberOfLegs / 2.0;

    return (
        <div
            datatype="WidescreenMatchStatistics"
            className="d-flex flex-row position-absolute top-0 left-0 right-0 bottom-0 p-3 bg-white">
            <div
                datatype="home-column"
                className="d-flex flex-column flex-grow-1 flex-shrink-1 flex-basis-0 position-relative">
                ...
                <WidescreenSaygPlayerStatistic
                    ...
                    setOneDartAverage={async (op: boolean) =>
                        setOneDartAverage(op)
                    }
                />
            ...
            {singlePlayer ? null : (
                <>
                    ...
                    <div
                        datatype="away-column"
                        className="d-flex flex-column flex-grow-1 flex-shrink-1 flex-basis-0">
