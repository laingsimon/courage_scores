...
export function MatchStatistics({
    ...
    const finished: boolean =
        (homeScore || 0) > numberOfLegs / 2.0 ||
        (awayScore || 0) > numberOfLegs / 2.0;
    const isSubscribed: boolean = !!(saygId && subscriptions[saygId]);
    const legDisplayOptions: ILegDisplayOptionsLookup =
        isSubscribed && !finished
            ? getLegDisplayOptions(legs, true)
            : legDisplayOptionsState;

    function getLegDisplayOptions(
        legs: { [key: number]: LegDto },
        showThrowsOnLastLeg?: boolean,
    ): ILegDisplayOptionsLookup {
    ...
    function updateLegDisplayOptions(
        legIndex: number,
        options: ILegDisplayOptions,
    ) {
        const newLegDisplayOptions: ILegDisplayOptionsLookup = Object.assign(
            {},
            legDisplayOptions,
        );
            ...
            <table className="table">
                <thead>
                    {singlePlayer ? null : (
                        <tr>
                            ...
                            <th
                                className={
                                    homeScore > ifUndefined(awayScore)
                                        ? 'text-primary'
                                        : ''
                                }>
                                {home}
                ...
                <tbody>
                    <tr>
                        <td>Score</td>
                        <td
                            className={`${!singlePlayer && homeScore > ifUndefined(awayScore) ? 'bg-winner text-primary' : ''} text-center`}>
                            <strong>{homeScore || '0'}</strong>
                        </td>
                        {singlePlayer ? null : (
                            <td
                                className={`${homeScore > ifUndefined(awayScore) ? '' : 'bg-winner text-primary'} text-center`}>
                    ...
                    {Object.keys(legs).map((legIndex: string) => {
                        ...
                        return (
                            <LegStatistics
                                ...
                                legDisplayOptions={
                                    legDisplayOptions[legKey] ||
                                    getLegDisplayOptions(legs)[legKey]
                                }
                                updateLegDisplayOptions={
                                    isSubscribed && !finished
                                        ? undefined
                                        : async (options: ILegDisplayOptions) =>
                                              updateLegDisplayOptions(
                                                  legKey,
                                                  options,
                                              )
                ...
                <tfoot>
                    <MatchAverage
                        homeAverage={
                            sumOf('home', 'score') /
                            (sumOf('home', 'noOfDarts') / 3)
                        }
                        awayAverage={
                            sumOf('away', 'score') /
                            (sumOf('away', 'noOfDarts') / 3)
                        }
                        singlePlayer={singlePlayer}
                        oneDartAverage={oneDartAverage}
                        setOneDartAverage={asyncCallback<boolean>(
                            setOneDartAverage,
                        )}
                    />
                    <MatchDartCount
