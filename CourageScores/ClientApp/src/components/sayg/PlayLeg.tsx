...
export function PlayLeg({
    ...
    const preferredStyle: string =
        getPreference<string>('sayg-style') || 'sayg-white';
    const [saygStyle, setSaygStyle] = useState<string>(preferredStyle);
    const { onError } = useApp();
    const { editScore, setEditScore } = useEditableSayg();
    const accumulator: LegCompetitorScoreDto = leg!.currentThrow
        ? leg![leg!.currentThrow]
        : null;
    const remainingScore: number = accumulator
        ? (leg!.startingScore || 0) - (accumulator.score || 0)
        : -1;
    const canEditPreviousCheckout: boolean =
        (!score &&
            previousLeg &&
            isEmpty(leg!.home.throws) &&
    ...
    const saygStyleOptions: IBootstrapDropdownItem[] = [
        {
            value: 'sayg-white',
            text: '🦺️ Black on white',
            collapsedText: '🦺️',
        },
        {
            value: 'sayg-black',
            text: '🦺️ White on black',
            collapsedText: '🦺️',
        },
    ];
    ...
    async function firstPlayerChanged(firstPlayerName: string) {
        ...
        const firstPlayer: IBootstrapDropdownItem = players.filter(
            (p) => p.value === firstPlayerName,
        )[0];
        const secondPlayer: IBootstrapDropdownItem = players.filter(
            (p) => p.value !== firstPlayerName,
        )[0];

        newLeg.playerSequence = [
            toLegPlayerSequence(firstPlayer),
            toLegPlayerSequence(secondPlayer),
        ];
        newLeg.currentThrow = firstPlayer.value as string;
        await onChange(newLeg);
    }

    function toLegPlayerSequence(
        option: IBootstrapDropdownItem,
    ): LegPlayerSequenceDto {
        return {
    ...
    async function handleScore(value: string) {
        ...
        if (editScore) {
            const newPlayerScores: LegCompetitorScoreDto =
                await changeScore(score);
            const newRemainingScore: number =
                (leg!.startingScore || 0) - (newPlayerScores.score || 0);
            await setEditScore();
    ...
    async function changeScore(score: number): Promise<LegCompetitorScoreDto> {
        ...
        thr.score = score;
        playerThrows.score = getScoreFromThrows(
            newLeg.startingScore || 0,
            playerThrows.throws,
        );
    ...
    async function addThrow(score: number) {
        try {
            ...
            const remainingScore: number =
                (leg!.startingScore || 0) - ((accumulator.score || 0) + score);
        ...
        return (
            <div datatype="change-checkout">
                <div className="alert alert-info text-center bottom-0 left-0 right-0">
                    <div>
                        <b>{previousWinner === 'home' ? home : 'away'}</b>{' '}
                        checked out <b>{lastThrow.score}</b> with{' '}
                        <b>{lastThrow.noOfDarts}</b> dart/s.
                    </div>
                    <div className="text-center">
                        <button
                            className="btn btn-primary"
                            onClick={() => setShowCheckout(previousWinner)}>
                            Change
                        </button>
                    </div>
                </div>
        ...
        <div
            ...
            {showWhoPlaysNextPrompt ? (
                <div className="text-center" datatype="bull-up">
                    ...
                    {playerOptions().map((op: IBootstrapDropdownItem) => (
                        <button
                            key={op.value}
                            className="btn btn-primary margin-right"
                            onClick={() =>
                                firstPlayerChanged(op.value as string)
                            }>
                            🎯
                            <br />
                            {op.text}
            ...
            {canEditPreviousCheckout && !showWhoPlaysNextPrompt
                ? renderEditCheckoutDarts()
                : null}
            {!showWhoPlaysNextPrompt ? (
                <div className={editScore ? ' bg-warning' : ''}>
                    <NumberKeyboard
                        value={score}
                        maxValue={180}
