...
export function WidescreenSaygPlayerStatistic({
    legs,
    player,
    oneDartAverage,
    setOneDartAverage,
}: IWidescreenSaygPlayerStatisticProps) {
    const orderedLegKeys = Object.keys(legs).sort(
    ...
    if (!lastLeg) {
        return (
            <div
                datatype="WidescreenSaygPlayerStatistic"
                className="d-flex flex-row justify-content-between text-center border-top border-secondary fs-3">
                Not started
            </div>
        );
    }
    ...
    return (
        <div
            ...
            <div onClick={() => setOneDartAverage(!oneDartAverage)}>
                ...
                {ifNaN(
                    round2dp(
                        lastLeg[player].score /
                            (lastLeg[player].noOfDarts / 3) /
                            dartAverageMultiplier,
                    ),
                    '-',
                )}
                <sup className="text-secondary-50">
                    {oneDartAverage ? '1' : '3'}
                </sup>
            </div>
            <div onClick={() => setOneDartAverage(!oneDartAverage)}>
