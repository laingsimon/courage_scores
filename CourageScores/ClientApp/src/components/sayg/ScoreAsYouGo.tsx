...
export function ScoreAsYouGo({
    ...
    function addLeg(legIndex: number): ScoreAsYouGoDto {
        ...
        const playerSequence =
            legIndex === 0 || singlePlayer ? getPlayerSequence() : [];
    ...
    function getPlayerSequence(): LegPlayerSequenceDto[] {
        if (singlePlayer) {
            return [
                ...
                { value: 'away', text: 'unused-single-player' },
            ];
        }
    ...
    async function recordWinner(winnerName: string, currentLeg: LegDto) {
        try {
            const newHomeScore: number =
                winnerName === 'home' ? (homeScore || 0) + 1 : homeScore || 0;
            const newAwayScore: number =
            ...
            if (!currentLeg.isLastLeg && !unbeatable) {
                ...
                if (!singlePlayer) {
                    newLeg.playerSequence = [
                        currentLeg.playerSequence![1],
                        currentLeg.playerSequence![0],
                    ];
            ...
            if (
                !hasFinished() &&
                hasFinished(newHomeScore, newAwayScore) &&
                onFinished
            ) {
                await onFinished();
            }
    ...
    function hasFinished(home?: number, away?: number) {
        return (
            (home || homeScore || 0) > numberOfLegs / 2.0 ||
            (away || awayScore || 0) > numberOfLegs / 2.0
        );
    }
    ...
    if (
        ...
        (!singlePlayer && (legIndex === numberOfLegs || hasFinished()))
    ) {
        if (useWidescreenStatistics) {
            return (
                <WidescreenMatchStatistics
                    ...
                    changeStatisticsView={asyncCallback(
                        setUseWidescreenStatistics,
                    )}
