...
export function EditSeason({
    ...
    async function deleteSeason() {
        ...
        if (
            !window.confirm(
                `Are you sure you want to delete the ${data.name} season?`,
            )
        ) {
            return;
        }
    ...
    async function toggleDivision(divisionId: string) {
        ...
        if (isDivisionSelected(divisionId)) {
            newData.divisionIds = newData.divisionIds!.filter(
                (id) => id !== divisionId,
            );
        } else {
            newData.divisionIds = (newData.divisionIds || []).concat(
                divisionId,
            );
        }
                ...
                <div className="input-group margin-right mb-3">
                    <div className="input-group-prepend">
                        <span className="input-group-text">
                            Use teams from season
                        </span>
                    </div>
                    <BootstrapDropdown
                        value={data.copyTeamsFromSeasonId}
            ...
            <div>
                ...
                <ul className="list-group mb-3">
                    {divisions.sort(sortBy('name')).map((d) => (
                        <li
                            key={d.id}
                            className={`list-group-item ${isDivisionSelected(d.id) ? 'active' : ''}`}
                            onClick={async () => await toggleDivision(d.id)}>
                            {d.name}
                        </li>
                    ))}
                </ul>
