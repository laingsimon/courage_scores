...
export function DivisionReports() {
    ...
    const [reportData, setReportData] = useState<ReportCollectionDto | null>(
        null,
    );
    ...
    const activeReport: string | undefined = getParameterDefault<
        string | undefined
    >('report', undefined, (x: string) => x);
    const { setTitle } = useBranding();

    function getParameterDefault<T>(
        name: string,
        defaultValue: T,
    ...
    function setParameter(name: string, value: string) {
        ...
        navigate(
            `/division/${divisionName}/reports/${season!.name}/?${newSearch}`,
        );
    }
    ...
    async function getReports() {
        ...
        try {
            ...
            const result: ReportCollectionDto =
                await reportApi.getReport(request);
            setReportData(result);
            if (result.reports && any(result.reports)) {
                const selectedReportExists: boolean = any(
                    result.reports,
                    (r: ReportDto) => r.name === activeReport,
                );
    ...
    return (
        <div className="content-background p-3">
            <PrintDivisionHeading
                hideDivision={(report && !report.thisDivisionOnly) || false}
            />
            <div className="input-group d-print-none">
                <div className="input-group-prepend">
                    <label htmlFor="topCount" className="input-group-text">
                        Return top{' '}
                    </label>
                </div>
                <input
                ...
                <button onClick={getReports} className="btn btn-primary">
                    {gettingData ? <LoadingSpinnerSmall /> : '📊 '}
                    Get reports...
                </button>
            </div>
            <div>
                {reportData && !gettingData && reportData.messages ? (
                    <ReportGenerationMessages messages={reportData.messages} />
                ) : null}
                {reportData && !gettingData ? renderReportNames() : null}
                {activeReport ? (
                    <div className="d-screen-none">
                        <strong className="fs-3 float-left">
                            {activeReport}
                        </strong>
                    </div>
                ) : null}
                {report && !gettingData ? (
                    <Report rows={report.rows!} columns={report.columns!} />
                ) : null}
            </div>
        </div>
    );
