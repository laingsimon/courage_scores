...
/* eslint-disable @typescript-eslint/no-explicit-any */
export function optionallyInvertFilter<T>(getFilter: (constraint: string | undefined, context: IRenderContext, fixtures: any) => IFilter<T>, filterInput?: string, renderContext?: any, fixtures?: DivisionFixtureDateDto[]): IFilter<T> {
...
export function getDateFilter(date: string | undefined, renderContext: IRenderContext, fixtures: DivisionFixtureDateDto[]): IFilter<IEditableDivisionFixtureDateDto> {
    switch (date) {
        case 'past':
    ...
    switch (type) {
        case 'league':
            return new AndFilter<IFixtureMapping>([
                ...
                new Filter<IFixtureMapping>(c => (c.fixture && !c.fixture?.isKnockout) || false),
            ]);
        case 'qualifier':
            return new Filter<IFixtureMapping>(c => c.fixture?.isKnockout === true);
        case 'tournament':
...
export function getTeamFilter(name?: string): IFilter<IFixtureMapping> {
    ...
    return new OrFilter<IFixtureMapping>([
        new Filter<IFixtureMapping>((c: IFixtureMapping) => (c.fixture && c.fixture.homeTeam && (c.fixture.homeTeam.id === name || c.fixture.homeTeam.name.toLowerCase() === name.toLowerCase()) || false)),
        new Filter<IFixtureMapping>((c: IFixtureMapping) => (c.fixture && c.fixture.awayTeam && (c.fixture.awayTeam!.id === name || c.fixture.awayTeam!.name.toLowerCase() === name.toLowerCase()) || false)),
        new Filter<IFixtureMapping>((c: IFixtureMapping) => (c.tournamentFixture && any(c.tournamentFixture!.sides!, (s: TournamentSideDto) => s.teamId === name || s.name!.toLowerCase() === name.toLowerCase()) || false))
    ]);
}
...
export function getFixtureDateFilters(filter: { date?: string, notes?: string }, renderContext: IRenderContext, fixtures?: DivisionFixtureDateDto[]): IFilter<IEditableDivisionFixtureDateDto> {
    return new AndFilter<IEditableDivisionFixtureDateDto>([
        new Filter<IEditableDivisionFixtureDateDto>((fd: IEditableDivisionFixtureDateDto) => any(fd.fixtures || []) || any(fd.tournamentFixtures || []) || any(fd.notes || []) || fd.isNew === true),
...
export interface IInitialisedFilters extends Record<string, string | undefined>{
    date?: string;
    type?: string;
    team?: string;
    notes?: string
