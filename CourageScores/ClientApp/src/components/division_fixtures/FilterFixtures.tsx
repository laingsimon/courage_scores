...
export function FilterFixtures() {
    ...
    if (filter.date && isEmpty(dateFilters, (f) => f.value === filter.date)) {
        if (filter.date.match(/\d{4}-\d{2}-\d{2}/)) {
            dateFilters.push(
                getFilterOption('date', renderDate(filter.date), filter.date),
            );
        } else {
    ...
    async function clearFavourites() {
        if (
            !window.confirm('Are you sure you want to clear your favourites?')
        ) {
            return;
        }
    ...
    return (
        <div className="mb-3" datatype="fixture-filters">
            ...
            {any(Object.keys(filter)) ? (
                <Link
                    className="btn btn-sm btn-outline-primary margin-left"
                    title="Clear all filters"
                    to={`${location.pathname}${clearFilters()}${location.hash}`}>
                    ➖
                </Link>
            ) : null}
        </div>
