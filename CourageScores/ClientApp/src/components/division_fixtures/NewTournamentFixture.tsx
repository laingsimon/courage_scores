...
export function NewTournamentFixture({
    date,
    tournamentFixtures,
    onTournamentChanged,
    allowCopySidesFrom,
    ...
    const [copySidesFrom, setCopySidesFrom] = useState<string | undefined>(
        undefined,
    );
    ...
    const addressOptions: IBootstrapDropdownItem[] = getCustomAddressItem(
        customAddress,
    ).concat(
        distinct(tournamentFixtures, 'address').map((f) => {
            return {
                text: f.proposed
                    ? f.address
                    : `⚠ ${f.address} (Already in use)`,
                value: f.address,
                className: f.proposed ? undefined : 'text-secondary',
            };
        }),
    );
    ...
    const divisionOptions: IBootstrapDropdownItem[] = [allDivisions].concat(
        divisions.map((d) => {
            return {
                text: d.name,
                value: d.id,
    ...
    function getTypeName(
        ...
        const uniqueFixtureType: TournamentGameDto[] = distinct(
            fixtureDate.tournamentFixtures,
            'type',
        );
        const notes: string[] =
            fixtureDate.notes?.map((n: FixtureDateNoteDto) => n.note) || [];
    ...
    function getSides(date: string): TournamentSideDto[] {
        const fixtureDate: DivisionFixtureDateDto | undefined =
            fixtureDates?.filter(
                (fd: DivisionFixtureDateDto) => fd.date === date,
            )[0];
        if (!fixtureDate) {
            return [];
        }
        ...
        return (
            fixtureDate.tournamentFixtures
                ?.filter(
                    (tf: DivisionTournamentFixtureDetailsDto) =>
                        !!tf.winningSide,
                )
                .map(
                    (tf: DivisionTournamentFixtureDetailsDto) =>
    ...
    function getType(date: string): string | undefined {
        ...
        return type ? `${type} final` : undefined;
    }
    ...
    async function createFixture() {
        ...
        if (
            (date > season!.endDate || date < season!.startDate) &&
            !confirm(
    ...
    return (
        <tr datatype="new-tournament-fixture">
            <td colSpan={5}>
                    ...
                    <Dialog title="Enter address">
                        <div className="input-group my-3">
                            ...
                            <input
                                ...
                                name="address"
                                value={customAddress || ''}
                                onChange={stateChanged(setCustomAddress)}
                            />
                        </div>
                        <div className="modal-footer px-0 pb-0">
                            <div className="left-aligned">
                                <button
                                    ...
                                    onClick={closeCustomAddressDialog}>
                                    Close
                                </button>
                            </div>
                            <button
