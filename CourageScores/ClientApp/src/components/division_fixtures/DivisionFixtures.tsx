...
export function DivisionFixtures({ setNewFixtures }: IDivisionFixturesProps) {
    ...
    const [editNote, setEditNote] = useState<EditFixtureDateNoteDto | null>(
        null,
    );
    const [showPlayers, setShowPlayers] = useState<{ [date: string]: boolean }>(
        getPlayersToShow(),
    );
    const [createFixturesDialogOpen, setCreateFixturesDialogOpen] =
        useState<boolean>(false);
    const [deletingAllFixtures, setDeletingAllFixtures] =
        useState<boolean>(false);
    ...
    function renderFixtureDate(fixtureDate: IEditableDivisionFixtureDateDto) {
        const allLeagueFixturesLength =
            fixtures?.filter((fd) => any(fd.fixtures, (f) => !f.isKnockout))
                .length ?? 0;
    ...
    function renderEditNote() {
        return (
            <EditNote
                note={editNote || {}}
                onNoteChanged={async (note: FixtureDateNoteDto) =>
                    setEditNote(note)
                }
    ...
    function scrollFixtureDateIntoView(date: string) {
        ...
        window.setTimeout(() => {
            ...
            const newFixtureDateElement = document.querySelector(
    ...
    async function addNewDate() {
        ...
        try {
            if (
                any(
                    fixtures,
                    (fd: DivisionFixtureDateDto) => fd.date === utcDate,
                )
            ) {
                return;
            }
            ...
            const newFixtureDate: IEditableDivisionFixtureDateDto =
                getNewFixtureDate(utcDate, isKnockout);
            await setNewFixtures(
                fixtures!.concat([newFixtureDate]).sort(sortBy('date')),
            );
        } finally {
    ...
    function renderNewDateDialog() {
        return (
            <Dialog title="Add a date to the season" slim={true}>
                <div className="pb-2">
                    ...
                    <input
                        ...
                        value={newDate}
                        onChange={stateChanged(setNewDate)}
                    />
                    {superleague ? null : (
                        <div className="form-check form-switch d-inline-block">
                            <input
                                ...
                                id="isKnockout"
                                checked={isKnockout}
                                onChange={stateChanged(setIsKnockout)}
                            />
                            <label
                ...
                <div className="modal-footer px-0 pb-0">
                    ...
                    <button className="btn btn-primary" onClick={addNewDate}>
                        Add date
                    </button>
                </div>
            </Dialog>
    ...
    function applyFixtureFilters(
        ...
        fixtureFilters: IFilter<IFixtureMapping>,
    ): DivisionFixtureDateDto {
        const filteredFixtureDate: DivisionFixtureDateDto = Object.assign(
            {},
            fixtureDate,
        );
        filteredFixtureDate.tournamentFixtures =
            fixtureDate.tournamentFixtures?.filter(
                (f: DivisionTournamentFixtureDetailsDto) =>
                    fixtureFilters.apply({
                        date: fixtureDate.date,
                        tournamentFixture: f,
                    } as IFixtureMapping),
            );
        const hasFixtures: boolean = any(
            fixtureDate.fixtures,
            (f: DivisionFixtureDto) => f.id !== f.homeTeam.id,
        );
        filteredFixtureDate.fixtures =
            !isAdmin && !hasFixtures
                ? []
                : fixtureDate.fixtures?.filter((f: DivisionFixtureDto) =>
                      fixtureFilters.apply({
                          date: fixtureDate.date,
                          fixture: f,
                      } as IFixtureMapping),
    ...
    async function bulkDeleteFixtures() {
        ...
        try {
            if (
                !confirm(
                    `Are you sure you want to delete all un-played league fixtures from ALL DIVISIONS in the ${season!.name} season?\n\nA dry-run of the deletion will run first.`,
                )
            ) {
                return;
            }
            ...
            const dryRunResult = await gameApi.deleteUnplayedLeagueFixtures(
                season!.id!,
                false,
            );
            ...
            if (
                !confirm(
                    `All the fixtures CAN be deleted without issue, are you sure you want to actually delete ${fixtureCount} fixtures from ${season!.name}?\n\n${dryRunResult.messages?.join('\n')}`,
                )
            ) {
                return;
            }

            const actualResult = await gameApi.deleteUnplayedLeagueFixtures(
                season!.id!,
                true,
            );
    ...
    try {
        ...
        const fixtureDateFilters: IFilter<DivisionFixtureDateDto> =
        ...
        return (
            <div className="content-background p-3">
                {isAdmin ? (
                    <div className="float-end" datatype="fixture-management-1">
                        {deletingAllFixtures ? null : (
                            <button
                        ...
                        {superleague || deletingAllFixtures ? null : (
                            <button
                                className="btn btn-primary margin-right"
                                onClick={() =>
                                    setCreateFixturesDialogOpen(true)
                                }>
                                🗓️ Create fixtures
                            </button>
                        )}
                ...
                {controls && !deletingAllFixtures ? <FilterFixtures /> : null}
                {isAdmin && newDateDialogOpen ? renderNewDateDialog() : null}
                <div>
                    {resultsToRender}
                    {isEmpty(resultsToRender, (f) => f != null) &&
                    any(fixtures) ? (
                        <div>No fixtures match your search</div>
                    ) : null}
                ...
                {isAdmin && createFixturesDialogOpen ? (
                    <CreateSeasonDialog
                        seasonId={season!.id!}
                        onClose={async () => setCreateFixturesDialogOpen(false)}
                    />
                ) : null}
                {isAdmin && !deletingAllFixtures ? (
                    <div className="mt-3" datatype="fixture-management-1">
                        <button
                            className="btn btn-primary margin-right"
                            onClick={() => setNewDateDialogOpen(true)}>
                            ➕ Add date
