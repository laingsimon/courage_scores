...
export function DivisionFixtureDate({
    ...
    async function toggleShowPlayers(date: string) {
        ...
        navigate({
            ...
            hash: any(Object.keys(newShowPlayers)) ? 'show-who-is-playing' : '',
        });
    }
    ...
    async function onUpdateFixtures(
        ...
        const newFixtures: DivisionFixtureDateDto[] = adaptFixtures(
            fixtures || [],
        );
    ...
    function isPotentialFixtureValid(fixture: DivisionFixtureDto) {
        ...
        if (
            any(
                date.tournamentFixtures,
                (t: DivisionTournamentFixtureDetailsDto) => !t.proposed,
            )
        ) {
            return false;
        }
        ...
        if (
            any(
                date.fixtures,
                (f: DivisionFixtureDto) => f.isKnockout || false,
            ) &&
            !fixture.awayTeam &&
            !canManageGames
        ) {
        ...
        const fixturesForThisTeam: DivisionFixtureDto[] = (date.fixtures || [])
            ...
            .filter(
                (f: DivisionFixtureDto) =>
                    ...
                    f.awayTeam?.id === fixture.homeTeam.id,
            );

        return !any(fixturesForThisTeam);
    }

    async function onChangeIsKnockout(
        event: React.ChangeEvent<HTMLInputElement>,
    ) {
        const newFixtureDate: IEditableDivisionFixtureDateDto = Object.assign(
            {},
            date,
        );
        ...
        if (
            ...
            newFixtureDate.fixtures = teams?.map(
                (team: DivisionTeamDto): IEditableDivisionFixtureDto => {
                    return {
                        id: team.id,
                        homeTeam: {
                            id: team.id,
        ...
        await setNewFixtures(
            (fixtures || []).map((fd: DivisionFixtureDateDto) =>
                fd.date === date.date ? newFixtureDate : fd,
            ),
        );
    }

    const hasKnockoutFixture: boolean = any(
        date.fixtures,
        (f: DivisionFixtureDto) =>
            f.id !== f.homeTeam.id && f.isKnockout === true,
    );
    const showQualifierToggle: boolean =
        canManageGames &&
        ((!hasKnockoutFixture &&
            !any(
                date.tournamentFixtures,
                (f: DivisionTournamentFixtureDetailsDto) => !f.proposed,
            ) &&
            !any(date.fixtures, (f) => f.id !== f.homeTeam.id)) ||
            date.isNew === true);
    const allowTournamentProposals: boolean = !any(
        date.fixtures,
        (f: DivisionFixtureDto) =>
            f.id !== f.homeTeam.id || !!f.awayTeam || f.isKnockout || false,
    );
    return (
        <div
            key={date.date}
            ...
            <h4>
                ...
                {isNoteAdmin ? (
                    <button
                        ...
                        onClick={() => startAddNote(date.date)}>
                        📌 Add note
                    </button>
                ) : null}
                {any(
                ...
                controls ? (
                    <span className="margin-left form-switch h6 text-body">
                        <input
                            ...
                            id={'showPlayers_' + date.date}
                            checked={showPlayers[date.date] || false}
                            onChange={() => toggleShowPlayers(date.date)}
                        />
                        <label
                            className="form-check-label margin-left"
                            htmlFor={'showPlayers_' + date.date}>
                            Who's playing?
                        </label>
                    </span>
                ) : null}
                {showQualifierToggle && !superleague ? (
                    <span className="margin-left form-switch h6 text-body">
                        <input
                            type="checkbox"
                            className="form-check-input align-baseline"
                ...
                any(date.tournamentFixtures, (t) => !!t.singleRound) ? (
                    <Link
                        ...
                        className="float-end fs-6 text-decoration-none">
                        🖥️
                    </Link>
                ) : null}
                {canAnalyseMatches &&
                ...
                any(date.tournamentFixtures, (t) => !t.proposed) ? (
                    <Link
                        to={`/analyse/${season!.name}/?${date
                            .tournamentFixtures!.filter((t) => !t.proposed)
                            .map((tf) => 't=' + tf.id)
                            .join('&')}`}
                        className="float-end fs-6 me-1 text-decoration-none">
                        📊
                    </Link>
                ) : null}
            </h4>
            {(date.notes || []).map((note: FixtureDateNoteDto) => (
                <FixtureDateNote
                    key={note.id}
                    note={note}
            ...
            <table className="table layout-fixed">
                <tbody>
                    {superleague
                        ? null
                        : date.fixtures
                              ?.filter(isPotentialFixtureValid)
                              .map((f: DivisionFixtureDto) => (
                                  <DivisionFixture
                                      key={f.id}
                                      fixture={f}
                                      date={date.date}
                    ...
                    {date.tournamentFixtures
                        ?.filter(
                            (t: DivisionTournamentFixtureDetailsDto) =>
                                !date.isKnockout && !t.proposed,
                        )
                        .map(
                            (
                    ...
                    {canManageTournaments && allowTournamentProposals ? (
