...
describe('WidescreenSaygRecentThrow', () => {
    ...
    async function getClassName(props) {
        await renderComponent({
            score: props.score || 1,
            bust: props.bust || false,
            throwNumber: props.throwNumber || 1,
        });
    ...
    describe('renders', () => {
        it('less than 100', async () => {
            const className = await getClassName({ score: 99 });

            expect(className).not.toContain('text-danger');
        });

        it('100+ in red', async () => {
            const className = await getClassName({ score: 100 });

            expect(className).toContain('text-danger');
        });

        it('180s in bold red', async () => {
            const className = await getClassName({ score: 180 });
        ...
        it('second most recent score with large font size', async () => {
            const className = await getClassName({ throwNumber: 2 });
        ...
        it('third most recent score with medium-large font size', async () => {
            const className = await getClassName({ throwNumber: 3 });
        ...
        it('fourth most recent score with medium font size', async () => {
            const className = await getClassName({ throwNumber: 4 });
        ...
        it('firth most recent score with large font size', async () => {
            const className = await getClassName({ throwNumber: 5 });
        ...
        it('sixth most recent score with large font size', async () => {
            const className = await getClassName({ throwNumber: 6 });
