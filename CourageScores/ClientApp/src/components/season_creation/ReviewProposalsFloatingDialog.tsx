...
export function ReviewProposalsFloatingDialog({
    proposalResult,
    ...
    const divisionOptions: IBootstrapDropdownItem[] = divisions
        .filter((d: DivisionDto) =>
            any(
                proposalResult.divisions,
                (proposedDivision) => proposedDivision.id === d.id,
            ),
        )
        .sort(sortBy('name'))
        .map((d: DivisionDto) => {
    ...
    try {
        ...
        const selectedDivisionIndex: number = divisionOptions
            .map((o: IBootstrapDropdownItem) => o.value)
            .indexOf(selectedDivisionId);
        const templateDivision: DivisionTemplateDto =
            template.divisions![selectedDivisionIndex];
        const placeholdersToRender: string[] = distinct(
        ...
        const templateSharedAddresses: string[] =
            template.sharedAddresses!.flatMap((a: string[]) => a);
        const divisionSharedAddresses: string[] =
            templateDivision.sharedAddresses!.flatMap((a: string[]) => a);
        return (
            <>
                <div
                    style={{ zIndex: '1051' }}
                    ...
                    <ul className="mt-3">
                        {placeholdersToRender.sort().map((key: string) => {
                            const mappedTeam: TeamDto | undefined =
                                proposalResult.placeholderMappings![key];
                            if (!mappedTeam) {
                                return null;
                            }

                            const isTemplateSharedAddress: boolean = any(
                                templateSharedAddresses,
                                (a: string) => a === key,
                            );
                            const isDivisionSharedAddress: boolean = any(
                    ...
                    <p>
                        Template:{' '}
                        <a
                            href={`/admin/templates/?select=${template.id}`}
                            target="_blank"
                            rel="noreferrer">
                            {template.name}
                        </a>
