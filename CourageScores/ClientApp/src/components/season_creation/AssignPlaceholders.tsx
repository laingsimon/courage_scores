...
export function AssignPlaceholders({
    ...
    const applicableDivisions: DivisionDto[] = divisions.filter(
        (division: DivisionDto) =>
            any(season.divisions, (d: DivisionDto) => d.id === division.id),
    );
    const template: TemplateDto = selectedTemplate.result!;
    const templateSharedAddresses: string[] = template.sharedAddresses!.flatMap(
        (a: string[]) => a,
    );

    function getPlaceholdersForTemplateDivision(
        templateDivision: DivisionTemplateDto,
    ): string[] {
        return distinct(
            templateDivision.dates!.flatMap((d: DateTemplateDto) =>
                d
                    .fixtures!.flatMap((f: FixtureTemplateDto) => [
                        f.home,
                        f.away,
                    ])
                    .filter((placeholder: string | undefined) => !!placeholder),
            ),
        );
    }

    function getAddress(team?: TeamDto): string {
        return team?.address?.trim() || team?.name.trim() || team?.id || '';
    }
    ...
    function getTeamsWithUniqueAddresses(
        ...
        const randomlyAssign: IBootstrapDropdownItem = {
            value: '',
    ...
    function getTeamsWithSharedAddresses(
        division: DivisionDto,
        ...
        const automaticallyAssign: IBootstrapDropdownItem = {
            value: '',
    ...
    async function setSelectedPlaceholder(teamId: string, placeholder: string) {
        const newMappings: IPlaceholderMappings = Object.assign(
            {},
            placeholderMappings,
        );
        if (teamId) {
    ...
    async function setSelectedSharedAddressPlaceholder(
        ...
        const allSharedAddressPlaceholders: string[] =
            template.divisions![divisionIndex].sharedAddresses!.filter(
                (sa: string[]) => any(sa, (a: string) => a === placeholder),
            )[0] || [];
        const otherSharedAddressPlaceholders: string[] =
            allSharedAddressPlaceholders.filter(
    ...
    return (
        <div>
            {applicableDivisions
                .sort(sortBy('name'))
                .map((division: DivisionDto, index: number) => {
                    const templateDivision: DivisionTemplateDto =
                        template.divisions![index];
                    const templatePlaceholders: string[] =
                        getPlaceholdersForTemplateDivision(templateDivision);
                    const teamsWithUniqueAddresses: IBootstrapDropdownItem[] =
                        getTeamsWithUniqueAddresses(division);
                            ...
                            <ul>
                                {templatePlaceholders
                                    ...
                                    .map((placeholder: string) => {
                                        const divisionSharedAddresses: string[] =
                                            templateDivision.sharedAddresses!.filter(
                                                (sa: string[]) =>
                                                    any(
                                                        sa,
                                                        (a: string) =>
                                        ...
                                        const hasDivisionSharedAddress: boolean =
                                            divisionSharedAddresses.length > 1;
                                        const hasTemplateSharedAddress: boolean =
                                            any(
                                        ...
                                        const availableTeamsForPlaceholder: IBootstrapDropdownItem[] =
                                            teamsWithUniqueAddresses.filter(
                                                (o: IBootstrapDropdownItem) => {
                                        ...
                                        const teamsWithSharedAddresses: IBootstrapDropdownItem[] =
                                            getTeamsWithSharedAddresses(
                                                division,
                                                divisionSharedAddresses.length,
                                            );
                                        const availableSharedDivisionTeamsForPlaceholder: IBootstrapDropdownItem[] =
                                            teamsWithSharedAddresses.filter(
                                                (o: IBootstrapDropdownItem) => {
                                                    const selected: boolean =
                                                        any(
                                                            Object.values(
                                        ...
                                        return (
                                            <li key={placeholder}>
                                                <span
                                                    className={`width-20 d-inline-block text-center margin-right ${className}`}>
                                                    {placeholder}
                                                </span>
                                                {hasDivisionSharedAddress &&
                                                !hasTemplateSharedAddress ? (
                                                    <BootstrapDropdown
                                                        options={
                                                            availableSharedDivisionTeamsForPlaceholder
                                                        }
                                                        value={
                                                            selectedTeamId || ''
                                                        }
                                                        onChange={(
                                                            ...
                                                            setSelectedSharedAddressPlaceholder(
                                                                index,
                                                                id,
                                                                placeholder,
                                                            )
