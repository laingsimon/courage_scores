...
export function DataBrowser() {
    ...
    const [response, setResponse] = useState<
        | IClientActionResultDto<SingleDataResultDto[]>
        | IClientActionResultDto<object>
        | null
    >(null);
    ...
    async function fetchData() {
        ...
        if (
            lastRequest &&
            lastRequest.table === newRequest.table &&
            lastRequest.id === newRequest.id
        ) {
        ...
        try {
            ...
            const response:
                | IClientActionResultDto<object>
                | IClientActionResultDto<SingleDataResultDto[]> = id
    ...
    function getPages() {
        const noOfItems = (
            response as IClientActionResultDto<SingleDataResultDto[]>
        ).result!.length;
    ...
    function renderValue(value: string | object | number, depth: number) {
        if (
            ...
            return (
                <abbr title={value} onClick={() => window.alert(value)}>
                    {renderDate(value)}
                </abbr>
            );
        }
    ...
    function renderViewToggleOption(
        name: string,
        ...
        return (
            <table className="table table-sm">
                ...
                <tbody>
                    {Object.keys(data)
                        .filter((key) =>
                            shouldShowProperty(key, data[key], depth),
                        )
                        .map((key: string) => (
                            <tr key={key}>
    ...
    function renderResponse() {
        const maybeSingleResponse: IClientActionResultDto<
            object & { id: string }
        > = response as IClientActionResultDto<object & { id: string }>;
        if (
            maybeSingleResponse &&
            maybeSingleResponse.result &&
            maybeSingleResponse.result.id
        ) {
            return renderItem(
                (response as IClientActionResultDto<object>).result!,
                1,
            );
        }

        const pageIndex = search.has('page')
            ? Number.parseInt(search.get('page')!)
            : 0;
        const minIndexInclusive = pageIndex * pageSize;
        const maxIndexExclusive = minIndexInclusive + pageSize;
        const allItems = response as IClientActionResultDto<
            SingleDataResultDto[]
        >;
        ...
        return (
            <>
                <ul className="list-group">
                    {allItems.result!.map(
                        (item: SingleDataResultDto, index: number) =>
                ...
                <div
                    className="d-flex flex-wrap flex-shrink-0 flex-grow-1"
                    datatype="pages">
                    {getPages().map((index) => (
                        <Link
                            key={index + ''}
