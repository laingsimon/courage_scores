...
export function ExportData() {
    ...
    const [saveError, setSaveError] =
        useState<IClientActionResultDto<ExportDataResultDto> | null>(null);

    useEffect(
        () => {
            const selected: TableDto[] = tables!.filter(
                (t: TableDto) => t.canExport,
            );
            const newExportRequest: ExportDataRequestDto = Object.assign(
                {},
                exportRequest,
            );
            newExportRequest.tables = toDictionary(
                selected,
                (t: TableDto) => t.name,
                () => [],
            );
    ...
    return (
        <div className="content-background p-3">
            ...
            <div className="input-group mb-3">
                <div className="form-check form-switch input-group-prepend">
                    <input
                        ...
                        name="includeDeletedEntries"
                        id="includeDeletedEntries"
                        checked={exportRequest.includeDeletedEntries}
                        onChange={valueChanged(exportRequest, setExportRequest)}
                    />
                    <label
                        className="form-check-label"
                        htmlFor="includeDeletedEntries">
            ...
            <TableSelection
                allTables={tables!}
                selected={Object.keys(exportRequest.tables!)}
                onTableChange={tableChanged}
            ...
            <div>
                ...
                {zipContent && !exporting && !saveError ? (
                    <a
                        download="export.zip"
                        href={'data:application/zip;base64,' + zipContent}
