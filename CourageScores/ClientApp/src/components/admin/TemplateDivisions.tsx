...
export function TemplateDivisions({
    divisions,
    onUpdate,
    ...
    async function updateDivision(
        update: DivisionTemplateDto,
        updateIndex: number,
    ) {
        await onUpdate(
            divisions.map((a: DivisionTemplateDto, index: number) =>
    ...
    async function deleteDivision(index: number) {
        await onUpdate(
            divisions.filter(
                (_: DivisionTemplateDto, i: number) => index !== i,
            ),
        );
    }

    async function onCopyToDivision(
        sourceDivisionIndex: number,
        destinationDivisionIndex: number,
    ) {
        const sourceDivision: DivisionTemplateDto =
            divisions[sourceDivisionIndex];

        const newDivisions: DivisionTemplateDto[] = divisions.map(
            (division: DivisionTemplateDto, index: number) => {
                if (index === destinationDivisionIndex) {
                    const newPrefix: string = (
                        destinationDivisionIndex + 1
                    ).toString();
                    const newDivision: DivisionTemplateDto = Object.assign(
                        {},
                        division,
                    );
                    newDivision.dates = prefixDateMnemonics(
                        sourceDivision.dates!,
                        newPrefix,
                    );
                    newDivision.sharedAddresses = prefixSharedAddressMnemonics(
                        sourceDivision.sharedAddresses!,
                        newPrefix,
                    );
                    return newDivision;
                }
    ...
    function prefixDateMnemonics(
        dates: DateTemplateDto[],
        prefix: string,
    ): DateTemplateDto[] {
        return dates.map((d: DateTemplateDto): DateTemplateDto => {
            return {
                fixtures: d.fixtures!.map(
                    (f: FixtureTemplateDto): FixtureTemplateDto => {
                        return {
    ...
    function prefixSharedAddressMnemonics(
            ...
            {divisions.map((d: DivisionTemplateDto, index: number) => (
                <li className="list-group-item" key={index}>
                    <TemplateDivision
                        ...
                        onUpdate={(update: DivisionTemplateDto) =>
                            updateDivision(update, index)
                        }
                        templateSharedAddresses={templateSharedAddresses}
                        divisionCount={divisions.length}
                        onCopyToDivision={async (destIndex: number) =>
                            onCopyToDivision(index, destIndex)
                        }
                        highlight={highlight}
