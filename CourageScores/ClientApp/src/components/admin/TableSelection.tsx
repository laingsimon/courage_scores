...
export function TableSelection({
    ...
    function renderTable(table: TableDto) {
        if (
            (requireCanExport && !table.canExport) ||
            (requireCanImport && !table.canImport)
        ) {
            return (
                <li key={table.name} className="list-group-item disabled">
                    {table.name}
                </li>
            );
        }
        ...
        return (
            <li
                onClick={async () => await toggleTable(table)}
                key={table.name}
    ...
    return (
        <ul className="list-group mb-3">
            {allTables ? (
                allTables.sort(sortBy('name')).map((t) => renderTable(t))
            ) : (
