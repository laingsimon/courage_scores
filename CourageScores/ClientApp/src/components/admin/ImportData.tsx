...
export function ImportData() {
    ...
    const [response, setResponse] =
        useState<IClientActionResultDto<ImportDataResultDto> | null>(null);
    const [saveError, setSaveError] = useState<
        object | IClientActionResultDto<ImportDataResultDto> | undefined
    >(undefined);
    ...
    useEffect(
        () => {
            ...
            const selected: string[] = tables
                .filter((t: TableDto) => t.canImport)
                .map((t: TableDto) => t.name);
            setImportRequest(
                Object.assign({}, importRequest, {
                    tables: selected,
                }),
            );
        },
        ...
        try {
            ...
            if (response.success) {
                ...
                if (response.status === 500 && response.text) {
                    ...
                    setSaveError({
                        errors: [textResponse],
                    } as IClientActionResultDto<ImportDataResultDto>);
    ...
    return (
        <div className="content-background p-3">
            ...
            <div className="input-group mb-3">
                <div className="form-check form-switch input-group-prepend">
                    <input
                        ...
                        name="purgeData"
                        id="purgeData"
                        checked={importRequest.purgeData}
                        onChange={valueChanged(importRequest, setImportRequest)}
                    />
                    <label className="form-check-label" htmlFor="purgeData">
                        Purge data
                    </label>
                </div>
            </div>
            ...
            <TableSelection
                allTables={tables!}
                selected={importRequest.tables!}
                onTableChange={propChanged(
                    importRequest,
                    setImportRequest,
                    'tables',
                )}
            ...
            {response ? (
                <div className="py-3">
                    <h5>Output</h5>
                    <ul>
                        {Object.keys(response.result!.tables!).map(
                            (t: string) => (
                                <li key={t}>
                                    <strong>{t}</strong>:{' '}
                                    {response.result!.tables![t]} row/s imported
                                </li>
                            ),
                        )}
                    </ul>
                    ...
                    <div className="overflow-auto max-scroll-height">
                        {response.errors!.map((error, index) => (
                            <div key={index + '_error'} className="text-danger">
                                {error}
                            </div>
                        ))}
