...
export function TemplateDivision({
    ...
    function findMnemonicsThatNeverPlayAtTheSameVenueAcrossAnyDate(): string[][] {
        let allMnemonics: string[] = distinct(
            division
                .dates!.flatMap((d: DateTemplateDto) => d.fixtures)
                .flatMap((f: FixtureTemplateDto | undefined) => [
                    f?.home,
                    f?.away,
                ])
        ...
        for (const mnemonic of allMnemonics) {
            let mnemonicsThatArePlayingAlwaysAtDifferentVenues: string[] =
                allMnemonics.filter((m: string) => !!m); // copy the array of all mnemonics

            for (const date of division.dates!) {
                const mnemonicsThatAreAtHome: string[] = date.fixtures!.map(
                    (f: FixtureTemplateDto) => f.home,
                );
                if (!any(mnemonicsThatAreAtHome, (m) => m === mnemonic)) {
                    continue;
                }

                mnemonicsThatArePlayingAlwaysAtDifferentVenues =
                    mnemonicsThatArePlayingAlwaysAtDifferentVenues.filter(
                        (m: string) =>
                            !any(
                                mnemonicsThatAreAtHome,
                                (atHomeOnDate: string) => atHomeOnDate === m,
                            ),
                    );
            }
            ...
            if (any(mnemonicsThatArePlayingAlwaysAtDifferentVenues)) {
                ...
                allMnemonics = allMnemonics.filter(
                    (m: string) =>
                        m !== mnemonic &&
                        !any(
                            mnemonicsThatArePlayingAlwaysAtDifferentVenues,
                            (addedMnemonic) => addedMnemonic === m,
                        ),
                );
            }
        }
        ...
        <div>
            ...
            {expanded ? (
                <SharedAddresses
                    ...
                    mnemonicsThatCanShareAddresses={findMnemonicsThatNeverPlayAtTheSameVenueAcrossAnyDate()}
                />
            ) : null}
            {expanded ? (
                <TemplateDates
                    ...
                    divisionSharedAddresses={division.sharedAddresses!.flatMap(
                        (a: string[]) => a,
                    )}
