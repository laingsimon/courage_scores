...
export function Templates() {
    ...
    const [saveError, setSaveError] =
        useState<IClientActionResultDto<TemplateDto> | null>(null);
    ...
    async function loadTemplates() {
        try {
            ...
            const idish: string | null = search.has('select')
                ? search.get('select')
                : null;
            if (!selected && idish) {
                const templateToSelect: TemplateDto = templates.filter(
    ...
    async function refreshHealth() {
        ...
        const response: IClientActionResultDto<SeasonHealthCheckResultDto> =
            await templateApi.health(selected!);
            ...
            <ul className="list-group mb-2" datatype="templates">
                {templates!.map((t) => (
                    <li
                        ...
                        {t.description ? (
                            <small className="mb-1">{t.description}</small>
                        ) : null}
                    </li>
                ))}
            </ul>
    ...
    function renderBadge(templateHealth?: SeasonHealthCheckResultDto) {
        ...
        const success: number = Object.values(templateHealth.checks!).filter(
            (c) => c.success,
        ).length;
        const fail: number = Object.values(templateHealth.checks!).filter(
            (c) => !c.success && c.errors!.length === 0,
        ).length;
        const error: number =
            Object.values(templateHealth.checks!).filter(
                (c) => c.errors!.length > 0,
            ).length + templateHealth.errors!.length;

        return (
            <span>
                {success ? (
                    <span className="badge rounded-pill bg-success margin-left">
                        {success}
                    </span>
                ) : null}
                {fail ? (
                    <span className="badge rounded-pill bg-warning margin-left">
                        {fail}
                    </span>
                ) : null}
                {error ? (
                    <span className="badge rounded-pill bg-danger margin-left">
                        {error}
                    </span>
                ) : null}
            </span>
    ...
    async function saveTemplate() {
        ...
        try {
            ...
            const result: IClientActionResultDto<TemplateDto> =
        ...
        return (
            <div className="content-background p-3">
                ...
                {loading || loading === null ? (
                    <p>
                        <LoadingSpinnerSmall /> Loading templates...
                    </p>
                ) : (
                    renderTemplates()
                )}
                {selected ? (
                    <div>
                        ...
                        {selected.templateHealth ? (
                            <div className="alert alert-success mt-3 p-1 pt-3">
                                <ViewHealthCheck
                                    result={selected.templateHealth}
                                />
                            </div>
                        ) : null}
                    </div>
