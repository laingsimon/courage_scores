...
export function MergeHiCheckAnd180s({
    fixtureData,
    data,
    setFixtureData,
    ...
    try {
        const oneEightiesNotRecorded: boolean = isEmpty(
            fixtureData.oneEighties,
        );
        const hiChecksNotRecorded: boolean = isEmpty(
            fixtureData.over100Checkouts,
        );

        return (
            <>
                {oneEightiesNotRecorded &&
                (any(getRecordsToMerge('home', 'oneEighties')) ||
                    any(getRecordsToMerge('away', 'oneEighties'))) ? (
                    <>
                        <tr>
                        ...
                        <tr datatype="merge-180s">
                            <td colSpan={2} className="text-end">
                                {oneEightiesNotRecorded &&
                                any(
                                    getRecordsToMerge('home', 'oneEighties'),
                                ) ? (
                                    <div datatype="home-180s">
                                        <h6>
                                            <button
                                                className="btn btn-sm btn-success margin-left"
                                                onClick={async () =>
                                                    await mergeRecords(
                                                        'home',
                                                        'oneEighties',
                                        ...
                                        <ol className="d-inline-block">
                                            {getRecordsToMerge<GamePlayerDto>(
                                                'home',
                                                'oneEighties',
                                            ).map((rec: GamePlayerDto) => (
                ...
                {hiChecksNotRecorded &&
                (any(getRecordsToMerge('home', 'over100Checkouts')) ||
                    any(getRecordsToMerge('away', 'over100Checkouts'))) ? (
                    <>
                        <tr>
                            <td colSpan={5} className="text-center">
                            ...
                            <td colSpan={2} className="text-end">
                                ...
                                any(
                                    ...
                                    <div datatype="home-hichecks">
                                        ...
                                        <ol className="d-inline-block">
                                            {getRecordsToMerge<NotablePlayerDto>(
                                                'home',
                                                'over100Checkouts',
                                            ).map((rec: NotablePlayerDto) => (
                                                <li key={rec.id}>
                                                    {rec.name} ({rec.score})
                                                </li>
                                            ))}
