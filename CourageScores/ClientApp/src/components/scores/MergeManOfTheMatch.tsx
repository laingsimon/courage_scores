...
export function MergeManOfTheMatch({
    data,
    setData,
    allPlayers,
    ...
    function getName(playerId: string): string {
        const player: TeamPlayerDto = allPlayers.filter(
            (p) => p.id === playerId,
        )[0];
        return player.name;
    }
    ...
    return (
        <tr>
            {data.home.manOfTheMatch ? (
                <td colSpan={2}>Merged</td>
            ) : (
                <td colSpan={2} className="text-end">
                    {data.homeSubmission &&
                    data.homeSubmission.home.manOfTheMatch ? (
                        <button
                            ...
                            Use{' '}
                            {getName(data.homeSubmission.home.manOfTheMatch)}
                        </button>
                    ) : (
                        <button
            ...
            {data.away.manOfTheMatch ? (
                <td colSpan={2}>Merged</td>
            ) : (
                <td colSpan={2}>
                    {data.awaySubmission &&
                    data.awaySubmission.away.manOfTheMatch ? (
                        <button
                            className="btn btn-success btn-sm"
                            onClick={async () =>
                                await setManOfMatch(
                            ...
                            Use{' '}
                            {getName(data.awaySubmission.away.manOfTheMatch)}
                        </button>
                    ) : (
                        <button
                            className="btn btn-secondary btn-sm"
