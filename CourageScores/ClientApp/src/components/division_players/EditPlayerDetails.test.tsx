...
describe('EditPlayerDetails', () => {
                          ...
                          seasons: [
                              {
                                  seasonId: seasonId,
                                  players:
                                      cumulativeCreatedPlayers as TeamPlayerDto[],
                              },
                          ],
                          name: '',
                          address: '',
    ...
    describe('interactivity', () => {
        const division: DivisionDto = divisionBuilder('DIVISION').build();
        const otherDivision: DivisionDto =
            divisionBuilder('OTHER DIVISION').build();
        const season: SeasonDto = seasonBuilder('SEASON')
            .withDivision(division)
            .build();
        const team: TeamDto = teamBuilder('TEAM')
        ...
        it('identifies new player ignoring deleted team seasons', async () => {
            ...
            const teamWithDeletedSeason: TeamDto = teamBuilder(
                'TEAM WITH DELETED SEASON',
            )
                    ...
                    result: {
                        ...
                        seasons: teamWithDeletedSeason!.seasons!.map(
                            (ts: TeamSeasonDto) => {
                                ...
                                const newTeamSeason: TeamSeasonDto =
                                    Object.assign({}, ts);
                                for (const p of cumulativeCreatedPlayers) {
                                    newTeamSeason.players = ts.players!.concat(
                                        p as TeamPlayerDto,
                                    );
                                }
        ...
        it('updates existing player', async () => {
            ...
            await renderComponent(
                {
                    player: playerBuilder('NAME', playerId)
                        .captain()
                        .email('EMAIL')
                        .build(),
                    seasonId: season.id,
            ...
            expect(updatedPlayer!.playerDetails.newTeamId).toEqual(
                otherTeam.id,
            );
            expect(saved).not.toBeNull();
