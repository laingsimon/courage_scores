...
export class PlayedEngine implements ILayoutEngine {
    ...
    calculate(request: ILayoutRequest): ILayoutDataForRound[] {
        const unplayedRounds: ILayoutDataForRound[] =
            this._unplayedEngine.calculate(request);
        const rounds: TournamentRoundDto[] = this.flattenAllRounds(request);
        let remainingSides: TournamentSideDto[] = request.sides.filter(
            (s) => !!s,
        );

        return unplayedRounds.map(
            (
                unplayedRound: ILayoutDataForRound,
                index: number,
            ): ILayoutDataForRound => {
                const playedRound: TournamentRoundDto = rounds[index];
                if (!playedRound) {
                    const adaptedRound: ILayoutDataForRound = Object.assign(
                        {},
                        unplayedRound,
                    );
                    adaptedRound.possibleSides = remainingSides.filter(
                        (s) => !!s,
                    ); // copy the array so it cannot be modified
                    return adaptedRound;
                }

                const winners: TournamentSideDto[] = [];
                const round: ILayoutDataForRound = this.createRound(
                    request.context!,
                    playedRound,
                    unplayedRound,
                    winners,
                    remainingSides,
                    unplayedRounds[index + 1],
                );
                const unselectedSides: TournamentSideDto[] =
                    remainingSides.filter(
                        (remainingSide: TournamentSideDto) => {
                            return !any(
                                round.alreadySelectedSides,
                                (s: TournamentSideDto) =>
                                    s.id === remainingSide.id,
                            ); // exclude any already selected side
                        },
                    );
    ...
    private createRound(
        ...
        playedRound: TournamentRoundDto,
        unplayedRound: ILayoutDataForRound,
        winners: TournamentSideDto[],
        remainingSides: TournamentSideDto[],
        nextRound?: ILayoutDataForRound,
    ): ILayoutDataForRound {
        const alreadySelectedSides: TournamentSideDto[] =
            unplayedRound.alreadySelectedSides || []; // sides will be added in as matches are created
        ...
        return {
            ...
            matches: unplayedRound.matches
                .map(
                    (
                        unplayedMatch: ILayoutDataForMatch,
                        index: number,
                    ): ILayoutDataForMatch => {
                        const playedMatch: TournamentMatchDto | null =
                            playedRound ? playedRound.matches![index] : null;
                        if (!playedMatch) {
                            // add to unplayed sides
                            const unplayedMatchWithoutOnTheNightMnemonic: ILayoutDataForMatch =
                                Object.assign({}, unplayedMatch);
                            if (
                                any(
                                    playedRound.matches || [],
                                ...
                                unplayedMatchWithoutOnTheNightMnemonic.numberOfSidesOnTheNight =
                                    undefined;
                            }
                ...
                .concat(
                    this.getExtraMatches(
                        context,
                        playedRound,
                        unplayedRound.matches.length,
                        alreadySelectedSides,
                        winners,
                    ),
                ),
    ...
    private createMatch(
        ...
        playedRound: TournamentRoundDto,
        unplayedMatch: ILayoutDataForMatch,
        playedMatch: TournamentMatchDto,
        index: number,
        ...
        return {
            sideA: this.getSide(
                context,
                playedMatch.sideA,
                unplayedMatch.sideA.mnemonic,
            ),
            sideB: this.getSide(
                context,
                playedMatch.sideB,
                unplayedMatch.sideB.mnemonic,
            ),
            scoreA:
                (playedMatch.scoreA ? playedMatch.scoreA.toString() : null) ||
                '0',
            scoreB:
                (playedMatch.scoreB ? playedMatch.scoreB.toString() : null) ||
                '0',
            match: playedMatch,
            winner: winner,
    ...
    private getExtraMatches(
        ...
        const extraMatches: TournamentMatchDto[] = skip(
            playedRound.matches || [],
            offset,
        );

        return extraMatches.map(
            (
                playedMatch: TournamentMatchDto,
                extraMatchIndex: number,
            ): ILayoutDataForMatch => {
                const overallIndex: number = extraMatchIndex + offset;
                const winner: string | undefined = this.getMatchWinner(
                    playedRound.matchOptions![overallIndex] ||
                        context.matchOptionDefaults,
                    winners,
                    playedMatch,
                );
                if (playedMatch.sideA) {
                ...
                return {
                    ...
                    scoreA:
                        (playedMatch.scoreA
                            ? playedMatch.scoreA.toString()
                            : null) || '0',
                    scoreB:
                        (playedMatch.scoreB
                            ? playedMatch.scoreB.toString()
                            : null) || '0',
                    match: playedMatch,
                    winner: winner,
                    saygId: playedMatch.saygId,
    ...
    private getSide(
        ...
        return {
            ...
            mnemonic: side && side.id ? undefined : mnemonic,
        };
    }

    private setSidePlayingInNextRound(
        side: TournamentSideDto,
        nextRound?: ILayoutDataForRound,
        unplayedMatch?: ILayoutDataForMatch,
    ) {
