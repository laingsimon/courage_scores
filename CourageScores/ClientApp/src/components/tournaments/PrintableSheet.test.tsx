...
describe('PrintableSheet', () => {
    ...
    function getRounds() {
        return Array.from(
            context.container.querySelectorAll('div[datatype^="round-"]'),
        ).map((round) => {
            return {
                //element: round,
                oneEighties: round.querySelector('div[data-accolades="180s"]')
                    ? {
                          ...
                          players: Array.from(
                              round.querySelectorAll(
                                  'div[data-accolades="180s"] div',
                              ),
                          ).map((e) => e.textContent),
                      }
                    : null,
                hiChecks: round.querySelector('div[data-accolades="hi-checks"]')
                    ...
                    : null,
                heading: round.querySelector('h5[datatype="round-name"]')!
                    .textContent,
                matches: Array.from(
                    round.querySelectorAll('div[datatype="match"]'),
                    ...
                    setInfo('div[datatype="scoreA"]', 'scoreA', (e: Element) =>
                        e.textContent!.trim(),
                    );
                    setInfo('div[datatype="scoreB"]', 'scoreB', (e: Element) =>
                        e.textContent!.trim(),
                    );
    ...
    function getWhoIsPlaying<T>(selector: (e: Element) => T): T[] {
        return Array.from(
            context.container.querySelectorAll('div[datatype="playing"] li'),
        ).map(selector);
    }
    ...
    function getAccolades<T>(name: string, selector: (e: Element) => T): T[] {
        return Array.from(
            context.container.querySelectorAll(
                'div[data-accolades="' + name + '"] div',
            ),
        ).map(selector);
    }

    function getWinner(): { name: string; link?: string } {
        const winnerElement = context.container.querySelector(
            'div[datatype="winner"]',
        )!;
    ...
    describe('played tournament', () => {
        ...
        it('renders tournament with one round', async () => {
            ...
            expect(rounds[0].matches).toEqual([match('A', '1', 'B', '2', 'b')]);
        });
                ...
                .round((r) =>
                    r
                        ...
                        .round((r) =>
                            r
                                .withMatch((m) =>
                                    m.sideA(sideE, 2).sideB(sideB, 1),
                                )
                                .withMatchOption((o) => o.numberOfLegs(3))
                                .round((r) =>
                                    r
                                        .withMatch((m) =>
                                            m.sideA(sideC, 2).sideB(sideE, 1),
                                        )
                                        .withMatchOption((o) =>
                                            o.numberOfLegs(3),
                                        ),
                        ...
                        .round((r) =>
                            r
                                ...
                                .round((r) =>
                                    r
                                        .withMatch((m) =>
                                            m.sideA(sideB, 2).sideB(sideE, 1),
                                        )
                                        .withMatch((m) =>
                                            m.sideA(sideJ, 2).sideB(sideL, 1),
                                        )
                                        .withMatchOption((o) =>
                                            o.numberOfLegs(3),
                                        )
        ...
        it('does not render winner when insufficient legs played', async () => {
            ...
            await renderComponent(
                ...
                {},
                appProps(
                    { teams: [player2Team], divisions: [division] },
                    reportedError,
                ),
            );
        ...
        it('renders who is playing (singles)', async () => {
            await renderComponent(
                ...
                {},
                appProps(
                    { teams: [player1Team], divisions: [division] },
                    reportedError,
                ),
            );

            expect(getWhoIsPlaying(whoIsPlayingText)).toEqual([
                '1 - A',
                '2 - B',
            ]);
        ...
        it('renders who is playing (teams)', async () => {
            ...
            await renderComponent(
                ...
                {},
                appProps(
                    { teams: [noPlayerTeam], divisions: [division] },
                    reportedError,
                ),
            );
        ...
        it('renders who is playing with no shows', async () => {
            oneRoundTournament2Sides.sides!.push(
                sideBuilder('C').noShow().build(),
            );
        ...
        it('renders 180s', async () => {
            oneRoundTournament2Sides.oneEighties!.push(
                player1,
                player2,
                player1,
                player1,
            ...
            expect(getAccolades('180s', (d) => d.textContent)).toEqual([
                'PLAYER 1 x 3',
                'PLAYER 2 x 1',
            ]);
        ...
        it('renders hi checks', async () => {
            oneRoundTournament2Sides.over100Checkouts!.push(
                Object.assign({}, player1, { score: 100 }),
            );
            oneRoundTournament2Sides.over100Checkouts!.push(
                Object.assign({}, player2, { score: 120 }),
            );
            ...
            expect(getAccolades('hi-checks', (d) => d.textContent)).toEqual([
                'PLAYER 1 (100)',
                'PLAYER 2 (120)',
            ]);
        ...
        it('cannot set match side a when not permitted', async () => {
            ...
            await doClick(
                context.container.querySelector('div[datatype="sideA"]')!,
            );
        ...
        it('can edit match side a', async () => {
            ...
            await doSelectOption(
                dialog.querySelector(
                    'div.btn-group:nth-child(2) .dropdown-menu',
                ),
                'e',
            ); // side
            await doSelectOption(
                dialog.querySelector(
                    'div.btn-group:nth-child(4) .dropdown-menu',
                ),
                '2',
            ); // score
            ...
            expect(updatedTournament!.round!.matches![0].sideA.name).toEqual(
                'e',
            );
        ...
        it('can edit match side b', async () => {
            ...
            await doClick(
                context.container.querySelector('div[datatype="sideB"]')!,
            );
            const dialog = context.container.querySelector('div.modal-dialog')!;
            await doSelectOption(
                dialog.querySelector(
            ...
            expect(updatedTournament).not.toBeNull();
            expect(updatedTournament!.round!.matches![0].sideB.name).toEqual(
                'e',
            );
        ...
        it('can edit 180s', async () => {
            await renderComponent(
                containerProps
                    .withTournament(twoRoundTournament4Sides)
                    .withAllPlayers([player1])
                    .build(),
                { editable: true },
                appProps({}, reportedError),
            );
        ...
        it('can edit side', async () => {
            ...
            expect(updatedTournament).not.toBeNull();
            expect(
                updatedTournament!.sides!.map((s: TournamentSideDto) => s.name),
        ...
        it('can add a side', async () => {
            const tournamentData: TournamentGameDto = tournamentBuilder()
                .round((r) =>
                    r.withMatch((m) =>
                        m
                            .sideA(
                                sideBuilder('A').withPlayer(player1).build(),
                                1,
                            )
                            .sideB(
                                sideBuilder('B').withPlayer(player2).build(),
                                2,
                            ),
                    ),
                )
            ...
            await doClick(
                dialog.querySelector(
                    '.list-group li.list-group-item:not(.disabled)',
                )!,
            ); // select a player
        ...
        it('can remove a side', async () => {
            await renderComponent(
                containerProps
                    .withTournament(oneRoundTournament2Sides)
                    .withAlreadyPlaying({})
                    .build(),
                { editable: true },
                appProps({ account }, reportedError),
    ...
    describe('unplayed tournament', () => {
        ...
        it('renders tournament with 2 sides', async () => {
            await renderComponent(
                containerProps
                    .withTournament(sideAandBTournament)
                    .withAlreadyPlaying({})
                    .build(),
                {},
                appProps({}, reportedError),
        ...
        it('can add a side', async () => {
            await renderComponent(
                containerProps
                    .withTournament(emptyTournament)
                    .withAlreadyPlaying({})
                    .withAllPlayers([player1])
                    .build(),
                { editable: true },
            ...
            await doClick(
                dialog.querySelector('.list-group li.list-group-item')!,
            ); // select a player
            await doClick(findButton(dialog, 'Add'));
        ...
        it('can add sides from hint', async () => {
            ...
            await doClick(
                context.container.querySelector(
                    'div[datatype="add-sides-hint"] > span',
                )!,
            );
