...
export function MatchLogRow({
    ...
    try {
        return (
            <tr className={winner && showWinner ? 'bg-winner' : ''}>
                {legNo === 1 ? (
                    <td
                        rowSpan={noOfLegs}
                        className="align-middle bg-white page-break-avoid">
                        {player}
                    </td>
                ) : null}
                <td>{legNo}</td>
                <td datatype="actual-darts">
                    {sum(
                        accumulator.throws,
                        (thr: LegThrowDto) => thr.noOfDarts!,
                    )}
                </td>
                <td datatype="game-shot">
                    {winner && lastThrow ? lastThrow.score : null}
                </td>
                <td datatype="score-left">
                    {winner || !lastThrow
                        ? null
                        : leg.startingScore! -
                          getScoreFromThrows(
                ...
                {legNo === 1 ? (
                    <td
                        rowSpan={noOfLegs}
                        className="align-middle bg-white fw-bold text-danger">
                ...
                <td datatype="game-dart">
                    {winner && lastThrow ? lastThrow.noOfDarts : null}
                </td>
