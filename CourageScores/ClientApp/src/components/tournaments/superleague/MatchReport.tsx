...
export function MatchReport({
    ...
    try {
        return (
            <div className="page-break-after" datatype="match-report">
                <h2 className="text-center">SOMERSET DARTS ORGANISATION</h2>
                <h3 className="text-center">
                    {division ? division.name + ' ' : ''}({gender})
                </h3>
                <table className="table">
                    <thead>
                        <tr>
                            <th
                                colSpan={15 + (noOfThrows + 1) * 2}
                        ...
                        <tr>
                            ...
                            {repeat(noOfThrows + 1, (i) => (
                                <th key={`sideA_throwHeading_${i}`}>{i + 1}</th>
                            ))}
                            <th>AD</th>
                            <th>GS</th>
                            <th>SL</th>
                            ...
                            <th>Players Name</th>
                            {repeat(noOfThrows + 1, (i) => (
                                <th key={`sideB_throwHeading_${i}`}>{i + 1}</th>
                            ))}
                            <th>AD</th>
                            <th>GS</th>
                            <th>SL</th>
                ...
                <div className="d-flex flex-row justify-content-around">
                    <div className="p-5 text-center">
                        Legs won: {legsWon(saygMatches, 'home')}
                    </div>
                    <div className="p-5 text-center">
                        Legs won: {legsWon(saygMatches, 'away')}
                    </div>
                </div>
            </div>
        );
