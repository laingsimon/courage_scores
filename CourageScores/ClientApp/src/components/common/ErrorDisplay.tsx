...
> & {
    errors?:
        | (
              | string
              | IServerSidePropertyException
              | IServerSideValidationErrors
          )[]
        | IServerSideValidationErrors;
...
export function ErrorDisplay({
    ...
    function renderValidationErrors(
        errors: IServerSideValidationErrors,
        key?: number,
    ) {
        return (
            <ol className="text-danger" key={key}>
                {Object.keys(errors).map((key: string) => {
                    return (
                        <li key={key}>
                            ...
                            {errors[key].map(
                                (
                                    e: string | IServerSidePropertyException,
                                    index: number,
                                ) => (
                                    <p key={index}>{e.toString()}</p>
                                ),
                            )}
                        </li>
                    );
    ...
    function renderServerSideException(
        exc: IServerSideException,
        key?: number,
    ) {
        const type = exc.Type;
    ...
    function renderError(
        ...
        if (typeof e === 'string') {
            ...
            return (
                <p key={key + '_error'} className="text-danger">
                    {message}
                </p>
            );
        }
        ...
        if (serverSideException.Exception) {
            return renderServerSideException(
                serverSideException.Exception,
                key,
            );
        }
    ...
    return (
        <Dialog onClose={onClose} title={title || 'There was an error'}>
            <div>
                ...
                {errors && errors.length === undefined
                    ? renderValidationErrors(
                          errors as IServerSideValidationErrors,
                      )
                    : null}
                {warnings
                    ? warnings.map((w, index) => (
                          <p key={index + '_warning'} className="text-warning">
                              {w}
                          </p>
                      ))
                    : null}
                {messages
                    ? messages.map((m, index) => (
                          <p key={index + '_message'} className="text-primary">
                              {m}
                          </p>
                      ))
                    : null}
