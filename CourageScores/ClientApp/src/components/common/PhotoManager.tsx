...
export function PhotoManager({
    ...
    const myPhotos: PhotoReferenceDto[] = (photos || []).filter(
        (p: PhotoReferenceDto) => p.author === account?.name,
    );
    const photosToShow: PhotoReferenceDto[] = canViewAllPhotos
        ? photos || []
        : myPhotos;
    const showPhotoSize: boolean = !!(
        account &&
        account.access &&
        (account.access.viewAnyPhoto || account.access.deleteAnyPhoto)
    );

    function getDownloadAddress(
        photo: PhotoReferenceDto,
        height?: number,
    ): string {
        ...
        <Dialog title="Photos">
            <div className="list-group mb-2">
                ...
                {photosToShow.map((photo: PhotoReferenceDto) => (
                    <a
                        href={getDownloadAddress(photo)}
                        className="list-group-item ps-2"
                        target="_blank"
                        rel="noreferrer"
                        key={photo.id}
                        title={`${photo.fileName}: ${getFileSize(photo.fileSize!)}`}>
                        {canDeletePhoto(photo) ? (
                            <button
                                className="btn btn-sm btn-danger float-start margin-right"
                                onClick={async (event) =>
                                    await deletePhoto(event, photo.id!)
                                }>
                                {deleting === photo.id ? (
                                    <LoadingSpinnerSmall />
                                ) : (
            ...
            {canUploadPhotos ? (
                <>
                    <div
                        datatype="upload-control"
                        className="mb-2 text-center border-dashed border-1 border-dark"
                        onClick={(event: MouseEvent) =>
                            triggerFileClick(event, 'fromCamera')
                        }>
                    ...
                    {uploading ? null : (
                        <div
                            ...
                            className="mb-2 text-center border-dashed border-1 border-dark"
                            onClick={(event: MouseEvent) =>
                                triggerFileClick(event, 'fromDevice')
                            }>
                            <button className="border-0 p-2 text-center text-decoration-none text-dark bg-white">
                                {uploading ? (
                                    <LoadingSpinnerSmall />
