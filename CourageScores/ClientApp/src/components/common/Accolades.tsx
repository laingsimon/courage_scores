...
export function add180<T extends IAccoladeFixtureData>(
    fixtureData: T,
    setFixtureData: (newData: T) => any,
): (player: NotablePlayerDto | NotableTournamentPlayerDto) => UntypedPromise {
...
/* eslint-disable @typescript-eslint/no-explicit-any */
export function remove180<T extends IAccoladeFixtureData>(
    fixtureData: T,
    setFixtureData: (newData: T) => any,
): (playerId: string, index: number) => UntypedPromise {
...
/* eslint-disable @typescript-eslint/no-explicit-any */
export function addHiCheck<T extends IAccoladeFixtureData>(
    fixtureData: T,
    setFixtureData: (newData: T) => any,
): (
    player: NotablePlayerDto | NotableTournamentPlayerDto,
    score: number,
) => UntypedPromise {
    return async (
        player: NotablePlayerDto | NotableTournamentPlayerDto,
...
/* eslint-disable @typescript-eslint/no-explicit-any */
export function removeHiCheck<T extends IAccoladeFixtureData>(
    fixtureData: T,
    setFixtureData: (newData: T) => any,
): (playerId: string, index: number) => UntypedPromise {
    return async (_: string, index: number) => {
