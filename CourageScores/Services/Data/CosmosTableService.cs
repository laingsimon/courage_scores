...
public class CosmosTableService : ICosmosTableService
{
    ...
    private async Task<bool> CanExportDataType(CancellationToken token)
    {
        ...
        return await _accessService.HasAccess(user, AccessOption.ExportData, UserAccessContext.Admin(), token);
    }

    private async Task<bool> CanImportDataType(Type? dataType, CancellationToken token)
    {
        var user = await _userService.GetUser(token);
        var importData = await _accessService.HasAccess(user, AccessOption.ImportData, UserAccessContext.Admin(), token);
        if (!importData)
        {
            return false;
        }
        ...
        var userAccess = new UserAccessService(_accessService, UserAccessContext.Admin(), user);
