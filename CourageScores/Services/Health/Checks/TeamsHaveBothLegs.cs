...
public class TeamsHaveBothLegs : ISeasonHealthCheck
{
    ...
    public async Task<SeasonHealthCheckResult> RunCheck(IReadOnlyCollection<DivisionHealthDto> divisions, HealthCheckContext context)
    {
        return (await divisions
            .SelectAsync(CheckDivision).ToList())
            .Aggregate(
                new SeasonHealthCheckResult { Success = true },
                (prev, current) => prev.MergeWith(current));
    }

    private async Task<SeasonHealthCheckResult> CheckDivision(DivisionHealthDto division)
    {
    ...
    private async Task<SeasonHealthCheckResult> CheckTeam(DivisionHealthDto division, DivisionTeamDto team)
    {
    ...
    private Task<SeasonHealthCheckResult> CheckLegs(DivisionHealthDto division, IReadOnlyCollection<LeagueFixtureHealthDto> allFixtures, DivisionTeamDto team1, DivisionTeamDto team2)
    {
