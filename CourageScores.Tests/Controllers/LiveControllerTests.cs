...
public class LiveControllerTests
{
    ...
    [Test]
    public async Task GetUpdate_GivenNoLastUpdatedQueryStringAndNoLastModifiedHeader_CallsGetUpdateWithNullSince()
    {
        var id = Guid.NewGuid();
        _service
        ...
        await _controller.GetUpdate(id, LiveDataType.Sayg, null, _token);

        _service.Verify(s => s.GetUpdate(id, LiveDataType.Sayg, null, _token));
    }

    [Test]
    public async Task GetUpdate_GivenLastUpdatedQueryStringAndNoLastModifiedHeader_CallsGetUpdateWithQueryStringValue()
    {
        var id = Guid.NewGuid();
        var lastUpdatedQueryString = new DateTimeOffset(2001, 02, 03, 04, 05, 06, TimeSpan.Zero);
        _service
            .Setup(s => s.GetUpdate(id, LiveDataType.Sayg, It.IsAny<DateTimeOffset?>(), _token))
            .ReturnsAsync(new ActionResultDto<UpdatedDataDto?>());

        await _controller.GetUpdate(id, LiveDataType.Sayg, lastUpdatedQueryString, _token);

        _service.Verify(s => s.GetUpdate(id, LiveDataType.Sayg, lastUpdatedQueryString, _token));
    }

    [Test]
    public async Task GetUpdate_GivenNoLastUpdatedQueryStringAndLastModifiedHeader_CallsGetUpdateWithHeaderValue()
    {
        var id = Guid.NewGuid();
    ...
    [Test]
    public async Task GetUpdate_GivenLastUpdatedQueryStringAndLastModifiedHeader_CallsGetUpdateWithQueryStringValue()
    {
        var id = Guid.NewGuid();
        var lastUpdatedQueryString = new DateTimeOffset(2001, 02, 03, 04, 05, 06, TimeSpan.Zero);
        var lastUpdatedHeader = new DateTimeOffset(2002, 03, 04, 05, 06, 07, TimeSpan.Zero);
    ...
    public async Task GetUpdate_WhenNullResultFromService_ReturnsNotFound()
    {
        ...
        var lastUpdatedQueryString = new DateTimeOffset(2001, 02, 03, 04, 05, 06, TimeSpan.Zero);
        _service
            .Setup(s => s.GetUpdate(id, LiveDataType.Sayg, lastUpdatedQueryString, _token))
            .ReturnsAsync(new ActionResultDto<UpdatedDataDto?>());
