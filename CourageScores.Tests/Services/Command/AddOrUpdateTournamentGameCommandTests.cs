...
public class AddOrUpdateTournamentGameCommandTests
{
    ...
    public async Task ApplyUpdates_WhenNoLatestSeason_ReturnsUnsuccessful()
    {
        ...
        Assert.That(result.Messages, Is.EqualTo("Unable to add or update game, no season exists"));
    ...
    public async Task ApplyUpdates_WhenSaygNotFoundForMatch_RemovesIdAndWarns()
    {
        ...
        Assert.That(result.Messages, Is.EqualTo($"Could not find sayg session for match: Side 1 vs Side 2, session has been removed and will need to be re-created (was {saygId})"));
    }
