...
Function Update-PullRequestReviewComment($GitHubToken, $Repo, $PullRequestNumber, $CommentId, $Body)
{
    ...
    $Url="https://api.github.com/repos/$($Repo)/pulls/comments/$($Comment.id)"

    Write-Host "Sending PATCH request to $($Url) with body $($Body) for pull request #$($PullRequestNumber)"
...
Function Add-PullRequestReviewComment($GitHubToken, $Repo, $PullRequestNumber, $CommitId, $Path, $Side, $SubjectType, $Body)
{
    ...
    Write-Host "Sending POST pull request review comment for $($PullRequestNumber) with body`n`n$($Body)"
    ...
    if ($Response.StatusCode -ne 201) 
    {
        Write-Error "Error creating pull request review comment at url $($Url) for #$($PullRequestNumber)"
    }
}
