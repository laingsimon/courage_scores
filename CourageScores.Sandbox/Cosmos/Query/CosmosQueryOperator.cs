...
internal class CosmosQueryOperator : IQueryFilterOperator
{
    ...
    public IQueryFilterOperator Not()
    {
        return new NotCosmosQueryOperator(this);
    }
    ...
    private static object? GetValue<T>(Token token)
    {
        ...
        var parseMethod = typeof(T).GetMethod("Parse", BindingFlags.Public | BindingFlags.Static, null, CallingConventions.Any, [typeof(string)], null);
        if (parseMethod == null)
        {
